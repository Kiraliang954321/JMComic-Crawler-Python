...
@register("astrbot_plugin_jmcomic", "KiraLiang", "JM Comic 下载器 — 发送 JM 码下载漫画 ZIP", "1.1.0")
class JMPlugin(Star):
    ...
    async def _process_task(self, task):
        ...
        try:
            ...
            sent_ok = await self._send_result(task, album_name, zip_path, file_size)

            if sent_ok and config.get("delete_after_send", True) and os.path.isfile(zip_path):
                try:
                    os.remove(zip_path)
                except OSError:
                    pass
    ...
    async def _send_result(self, task, album_name: str, zip_path: str, file_size: int) -> bool:
        """主动发送下载完成的 ZIP 文件。返回 True 表示 ZIP 文件发送成功。"""
        size_mb = file_size / (1024 * 1024)
        msg_type = "GroupMessage" if task.chat_type == "group" else "PrivateMessage"
        ...
        try:
            await StarTools.send_message_by_id(
                type=msg_type,
                id=target_id,
                message_chain=MessageChain([Comp.Plain(f"📦 《{album_name}》下载完成 ({size_mb:.1f}MB)")]),
            )
        except Exception:
            import traceback
            traceback.print_exc()
        ...
        try:
            await StarTools.send_message_by_id(
                type=msg_type,
                id=target_id,
                message_chain=MessageChain([Comp.File(file=zip_path, name=f"{album_name}.zip")]),
            )
            return True
        except Exception as e:
            import traceback
            ...
            try:
                await StarTools.send_message_by_id(
                    type=msg_type,
                    id=target_id,
                    message_chain=MessageChain([Comp.Plain(f"⚠ 文件发送失败: {e}")]),
                )
            except Exception:
                pass
            return False
    ...
    async def _send_error(self, task, error: str):
        ...
        try:
            await StarTools.send_message_by_id(
                type=msg_type,
                id=target_id,
                message_chain=MessageChain([Comp.Plain(f"❌ JM{task.jm_id} 下载失败: {error}")]),
            )
        except Exception:
            import traceback
            traceback.print_exc()
